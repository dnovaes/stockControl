        ...
        companion object {
            ...
            fun naiveEncoder(img: Bitmap): Sequence<NiimbotPacket> = sequence {
                ...
                for (x in 0 until img.height) {
                    val currentTopLineDataIndex = ((x + 1) * bytesPerLine)
                    if (currentTopLineDataIndex > imgData.size)
                        break
                    val lineData = imgData.sliceArray(x * bytesPerLine until currentTopLineDataIndex)

                    val counts = Array(3) { i ->
                        countBitsOfBytes(lineData.sliceArray(i * (bytesPerLine/4) until (i + 1) * (bytesPerLine/4)))
                    }
